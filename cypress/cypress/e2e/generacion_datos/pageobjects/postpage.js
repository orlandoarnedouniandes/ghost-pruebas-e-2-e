class PostPage {
    fillandSavePostForm(postTitle, postContent,escenario = 'escenario',imagen = 'imagen'){
        cy.get('textarea.gh-editor-title').clear().type(postTitle, {force: true, parseSpecialCharSequences: false });
        cy.get('div.koenig-editor__editor').clear().type(postContent, {force: true, parseSpecialCharSequences: false});
        cy.screenshot(escenario+'/'+imagen);
        cy.wait(2000);
    }

    cleanandSavePostForm(escenario = 'escenario',imagen = 'imagen'){
        cy.get('textarea.gh-editor-title').type('a', {force: true, parseSpecialCharSequences: false }).clear();
        cy.get('div.koenig-editor__editor').type('a', {force: true, parseSpecialCharSequences: false }).clear();
        cy.screenshot(escenario+'/'+imagen);
        cy.wait(2000);
    }
