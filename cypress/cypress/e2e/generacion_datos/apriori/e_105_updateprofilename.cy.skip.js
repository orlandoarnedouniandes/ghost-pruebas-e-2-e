...
context("UpdateProfileName", function () {
    ...
    it("A priori - El usuario actualiza su nombre de autor", function () {
        ...
        cy.get('@slug').then((slug) => {
            cy.log('Slug: '+slug);
            // and the user logout
            this.page.logout(escenario, '5_logout');
            
            //Then the user verify the name was updated and the user page is displayed
            this.page.visit(this.data.url+'author/'+slug, escenario, '5_authorpage');
            this.sitePage.verifyIfUserNameIsDisplayed(name, escenario, '6_verify');
        });
    });
});