class PageObject {
    ...
    verifylastPageTitleandPublished(expectedTitle, escenario='escenario',imagen='imagen'){
        cy.get('h3.gh-content-entry-title').first().invoke('text').then((text) => {
            expect(text.trim()).to.eq(expectedTitle.trim());
        });
        cy.get('span.gh-content-status-published').first().invoke('text').then((text) => {
            expect(text.trim()).to.eq('Published');
        });
        cy.screenshot(escenario+'/'+imagen);
    }  
